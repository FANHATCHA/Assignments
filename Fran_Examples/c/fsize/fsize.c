...
# define STATFUNC(n,b) _stat(n, b)
# define STAT_ISDIR(m)		(((m) & _S_IFDIR) == _S_IFDIR)
# define DIRWALK(n, f)		dirwalkWindows(n, f)
#else
...
#ifdef OS_WINDOWS
int dirwalkWindows(const char *dir, int (*fsizeFunc)(const char *))
{
	char name[FILENAME_MAX], dirmask[FILENAME_MAX];
	_finddata_t findData;
	intptr_t findResult;

	if (strlen(dir) + 5 > sizeof(dirmask))
		fprintf(stderr, "dirwalk: dirname too long\n");
	sprintf(dirmask, "%s/*.*", dir);


	if ((findResult = _findfirst(dirmask, &findData)) == NULL)
	{
		fprintf(stderr, "No entries in %s\n", dirmask);
		return -1;
	}
	...
	do {
		...
		if (strlen(dir) + strlen(findData.name) + 2 > sizeof(name))
			fprintf(stderr, "dirwalk: name %s/%s too long\n",
					dir, findData.name);
		else {
			sprintf(name, "%s/%s", dir, findData.name);
			(*fsizeFunc)(name);
		}
	} while (_findnext(findResult, &findData) >= 0);

	/* test if we exitted successfully */
